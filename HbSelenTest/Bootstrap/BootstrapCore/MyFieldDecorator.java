...
public class MyFieldDecorator extends DefaultFieldDecorator {
	...
	private List<?> decorateList(ClassLoader loader, Field field) {
		...
		List<WebElement> listElements = proxyForListLocator(loader,
				createLocator(field));
		...
		if (IElement.class.isAssignableFrom(clazz)) {
			for (WebElement e : listElements) {
				elements.add(elementFactory.create(
						(Class<? extends IElement>) ((ParameterizedType) field
	...
	@SuppressWarnings("unchecked")
	private IContainer decorateContainer(final ClassLoader loader,
			final WebElement wrappedElement, final Class<?> clazz) {
		final IContainer container = containerFactory.create(
				(Class<IContainer>) clazz, wrappedElement);

		PageFactory.initElements(new MyFieldDecorator(wrappedElement),
				container);
		return container;
	}
	...
	private IContainer decorateContainer(final ClassLoader loader,
		...
		final WebElement wrappedElement = proxyForLocator(loader,
				createLocator(field));
		return decorateContainer(loader, wrappedElement,
				(Class<? extends IContainer>) field.getType());
	}
